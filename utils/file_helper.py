...
class FileHelper:
    ...
    def extract_raw(self, target_path: str):
        ...
        file_paths_by_create_times: {datetime: [Path]} = {}
        ...
        for key, values in file_paths_by_create_times.items():
            ...
            for index, file_path in enumerate(values):
                converted_file_name = f"{key.strftime("%Y%m%d_%H%M%S")}_{index + 1:03d}{file_path.suffix.lower()}"
